...
export class DashboardComponent implements OnInit {
  ...
  constructor(public apiSrvc: ApiService, private route: ActivatedRoute, private router: Router, private spinner: NgxSpinnerService,
    private ngbmodal: NgbModal, private ngbmodalActive: NgbActiveModal,private title: Title) {
    // alert('Calling')
    this.title.setTitle("XtractSales-SalesGross");
    let today = new Date();
    var lastDayOfMonth = new Date(today.getFullYear(), today.getMonth() + 1, 0);
    this.FromDate = today.getFullYear() + '-' + ("0" + (today.getMonth() + 1)).slice(-2) + '-01'
    this.ToDate = today.getFullYear() + '-' + ("0" + (today.getMonth() + 1)).slice(-2) + '-' + ("0" + (today.getDate() - 1)).slice(-2)
  ...
  getFavReports() {
    ...
    this.apiSrvc.postmethod('favouritereports/get', obj).subscribe(
      res => {
        if (res.status == 200) {
      ...
      totalres => {
        if (totalres.status == 200) {
          if (totalres.response != undefined) {
            this.TotalSalesGross = totalres.response.map(v => ({ ...v, Data2: [] }))
  ...
  openDetails(Item, ParentItem, cat) {
    ...
    if (cat == '3') {
      if (Item.data3 != undefined) {
        ...
        const DetailsSalesPeron = this.ngbmodal.open(SalesgrossDetailsComponent, {
          // size:'xl',
          backdrop: "static",
        });
