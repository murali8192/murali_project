...
export class SalesgrossReportsComponent implements OnInit {
  ...
  getReportsControl() {
    ...
    this.service.postmethod('reportcontrolvariable/get', Obj).subscribe(res => {
      this.FavReports = res.response.filter(e => e.RCV_SubModule_Id === 17);
      this.FavReports = this.FavReports.map(v => ({ ...v, Fav_Report_Value: '' }))
      // console.log(this.FavReports);
  ...
  saveFav() {
    ...
    var result = this.selectedDataGrouping.map(function (a) { return a.ARG_ID; });
    this.FavReports[0].Fav_Report_Value = result.toString()
    this.FavReports[1].Fav_Report_Value = this.fromDate.year + '-' + ('0' + this.fromDate.month).slice(-2) + '-' + ('0' + this.fromDate.day).slice(-2) + ',' + this.toDate.year + '-' + ('0' + this.toDate.month).slice(-2) + '-' + ('0' + this.toDate.day).slice(-2)
    this.FavReports[2].Fav_Report_Value = this.neworused.toString()
    this.FavReports[3].Fav_Report_Value = this.retailorlease.toString()
    this.FavReports[4].Fav_Report_Value = this.dealstatus.toString()
    this.FavReports[5].Fav_Report_Value = this.selecttarget.toString()
    this.FavReports[6].Fav_Report_Value = this.Transactorgl.toString()
    this.FavReports[7].Fav_Report_Value = this.includecharges.toString()
    this.FavReports[8].Fav_Report_Value = this.includeorexclude.toString()
    this.FavReports[9].Fav_Report_Value = this.toporbottom.toString()
    this.FavReports[10].Fav_Report_Value = selectedstorevalues.toString() == "" ? '0' : selectedstorevalues.toString()
    this.FavReports[11].Fav_Report_Value = selectedSalespersonvalues.toString() == "" ? '0' : selectedSalespersonvalues.toString()
    this.FavReports[12].Fav_Report_Value = selectedSalesManagersvalues.toString() == "" ? '0' : selectedSalesManagersvalues.toString()
    this.FavReports[13].Fav_Report_Value = selectedFiManagersvalues.toString() == "" ? '0' : selectedFiManagersvalues.toString()
    var resulttxt = this.selectedDataGrouping.map(function (a) { return a.ARG_LABEL; });
    this.FavReports[14].Fav_Report_Value = resulttxt.toString()
    var resultcolname = this.selectedDataGrouping.map(function (a) { return a.columnName; });
    this.FavReports[15].Fav_Report_Value = resultcolname.toString()

    if (this.favName != "") {
      this.componentState = false
      this.FavValid = false
      const obj = {
        "Fav_Name": this.favName,
        "Fav_User_Id": "1",
        "Fav_Module_Id": 17,
        "Fav_Module_Name": "Sales Gross",
        "Fav_Module_Path": "SalesGross",
        "Fav_Status": "Y"
      }
      this.service.postmethod('favourite', obj).subscribe(
        res => {
          if (res.status == 200) {
            var reportssaving: any = []
            this.FavReports.forEach(val => {
              reportssaving.push({
                "Fav_RCV_Id": val.RCV_Id,
                "Fav_Table_Id": res.response.Fav_id,
                "Fav_Report_Value": val.Fav_Report_Value,
                "Fav_Report_Status": "Y"
              });
            })
            // console.log(reportssaving)
            const obj = {
              "FavouriteReports": reportssaving
            }
            this.service.postmethod('favouritereports', obj).subscribe(
              Reportsres => {
