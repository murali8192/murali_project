...
export class DashboardComponent implements OnInit {
      ...
      res => {
        if (res.status == 200) {
          if (Block_val == "LI") {
            ...
            this.SalesGridData_excel = res.response.map(({ SNo, ...rest }) => ({ ...rest }));
            var sorted = {};
  ...
  FIReport(Block_val) {
    ...
    this.spinner.show();
    this.apiSrvc.postmethod('xtract/GetNightlyFIReport', obj).subscribe(res => {

      if (res.status == 200) {
        if (Block_val == "FI") {
          this.FIGridData = res.response;
          this.FIGridData_excel = res.response.map(({ SNo, ...rest }) => ({ ...rest }));
  ...
  DealesReport() {
    ...
    this.spinner.show();
    this.apiSrvc.postmethod('xtract/GetNightlyDealsReport', obj).subscribe(res => {
      if (res.status == 200) {
        this.DealesGridData = res.response;
        var Deales_Grid_VS_ID = res.response.map(({ VS_ID, ...rest }) => ({ ...rest }));
  ...
  SalesPersonReport() {
    ...
    this.apiSrvc.postmethod('xtract/GetNightlySalesPersonReport', obj).subscribe(res => {
      if (res.status == 200) {
        this.SalesPersonData = res.response;
        this.SalesPersonData_excel = res.response.map(({ AS_ID, ...rest }) => ({ ...rest }));
      }
    }, (error) => {
      console.log(error);
    },
