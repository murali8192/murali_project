...
export class StoresComponent implements OnInit {
  ...
  constructor(public apiSrvc: ApiService, private formBuilder: FormBuilder, private ngbmodal: NgbModal, private ngbmodalActive: NgbActiveModal,private spinner: NgxSpinnerService) {
    const data = {
      title: 'Stores',
      path1: '',
      path2: '',
  ...
  get form() {return this.UsersForm.controls; }
  ...
  getData() {
    ...
    this.apiSrvc.postmethodOne('axelone/GetAxelStores', Obj).subscribe(res => {
      this.StoresData = res.response.filter(e=>e.Xtract == 'Y');;
      this.spinner.hide();
