...
export class DashboardComponent implements OnInit {
    ...
    this.apiSrvc.postmethod('xtract/GetPartsGrossSummary', obj).subscribe(
      totalres => {
        if (totalres.status == 200) {
          this.TotalPartsGross = totalres.response.map(v => ({ ...v, Data2: [], DealerName: 'Reports Total', Dealer: '+' }))
          //  console.log(this.TotalPartsGross)
          this.spinner.hide();


          if (this.TotalPartsGross.length > 0) {
  ...
  openMenu() {
    // const modalRef = this.ngbmodal.open(PartsReportNewComponent, {
    //   size: 'xl',
    //   backdrop: "static",
    // });
  ...
  openDetails(Item, ParentItem,ref) {
   ...
   if(ref=='2'){
    ...
    const DetailsPartsPerson = this.ngbmodal.open(PartsgrossDetailsComponent, {
      // size:'xl',
      backdrop: "static",
    });
...
if (ref == '1') {
  if (Item.data1 != undefined && Item.data1 != 'Reports Total') {
    ...
    DetailsPartsPerson.result.then((data) => {
    }, (reason) => {
      // on dismiss
